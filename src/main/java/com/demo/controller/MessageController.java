...
public class MessageController implements CommunityConstant {
    ...
    public String getLetterDetail(@PathVariable("conversationId") String conversationId, Page page, Model model) {
        ...
        // 设置已读
        List<Integer> ids = getLetterIds(messageService.findLetters(conversationId, 0, Integer.MAX_VALUE));
        if (!ids.isEmpty()) {
    ...
    public String getNoticeDetail(@PathVariable("topic") String topic, Page page, Model model) {
        ...
        System.out.println(noticeVoList.toString());
        // 设置已读
        List<Integer> ids = getLetterIds(messageService.findNotices(user.getId(), topic, 0, Integer.MAX_VALUE));
        if (!ids.isEmpty()) {
            messageService.readMessage(ids);
        }
    ...
    private List<Integer> getLetterIds(List<Message> letterList) {
        ...
        if (letterList != null) {
            for (Message message : letterList) {
                if (hostHolder.getUserThreadLocal().getId() == message.getToId() && message.getMessageStatus() == 0) {
